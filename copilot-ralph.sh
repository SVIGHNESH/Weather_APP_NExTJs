...
echo "Starting Copilot Agent - Max $MAX iterations"
echo ""
...
for ((i=1; i<=$MAX; i++)); do
    ...
    result=$(copilot -p "You are Ralph, an autonomous coding agent. Do exactly ONE task per iteration.
...
- If tasks remain [ ], just end your response (next iteration will continue)" --allow-all-tools --silent)
